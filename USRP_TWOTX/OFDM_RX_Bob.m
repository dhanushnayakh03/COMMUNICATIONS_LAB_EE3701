...
Tx_total_pilots = [16,18,24,26,40,42,48,50];

Yp_payload1 = RX_Payload_1_Frequency(Tx_total_pilots).';  
Xp_payload1 = [1,-1,1,-1,1,-1,1,-1].';   % column vector

k_centered_payload1 = (Tx_total_pilots - 1).' - (N_FFT/2);
F_Tx_total = exp(-1j * 2*pi * (k_centered_payload1) * (0:7) / N_FFT);

h_Tx_total = F_Tx_total \ (Yp_payload1 ./ Xp_payload1);   % [8×1]

h_padded_e = [h_Tx_total(:); zeros(N_FFT - length(h_Tx_total), 1)];
H_Tx_total = fftshift(fft(h_padded_e)).';

% Regularize small values (preserving phase)
eps_val = 1e-12;
H_Tx_total = H_Tx_total + eps_val.*(abs(H_Tx_total)<eps_val);

H_Tx_total = H_Tx1_recon_1 + H_Tx2_recon_1 ; 

RX_Tx_Payload_1_Frequency_Eq = RX_Payload_1_Frequency ./ H_Tx_total;  % [1×N_FFT]
...
%% --- Zero-Forcing Equalization for Payload 1 ---
Tx_total_pilots = [16,18,24,26,40,42,48,50];
Xp_payload2 = [1,-1,1,-1,1,-1,1,-1]; 

Yp_payload2 = RX_Payload_2_Frequency(Tx_total_pilots).';   % column vector
k_centered_payload2 = (Tx_total_pilots - 1).' - (N_FFT/2);

% Create pilot matrix
F_Tx_total = exp(-1j * 2*pi * (k_centered_payload2) * (0:7) / N_FFT);

% Channel estimation (column vector)
h_Tx_total = F_Tx_total \ (Yp_payload2 ./ Xp_payload2.');

% Zero-pad to match FFT size (make sure both are column vectors)
h_padded_e = [h_Tx_total; zeros(N_FFT - length(h_Tx_total), 1)];

% Frequency response (row vector for division)
H_Tx_total = fftshift(fft(h_padded_e)).';

% Avoid near-zero values
H_Tx_total(abs(H_Tx_total) < 1e-12) = 1e-12;

H_Tx_total = H_Tx1_recon_2 + H_Tx2_recon_2 ; 

% Equalize (ensure both are row vectors)
RX_Tx_Payload_2_Frequency_Eq = RX_Payload_2_Frequency ./ H_Tx_total;
...
% Payload 1
RX_data_1_eq = RX_Tx_Payload_1_Frequency_Eq(data_idx); % [1 x N_data]
RX_data_1_demod = pskdemod(RX_data_1_eq, 4, pi/4); % QPSK demodulation

% Payload 2
RX_data_2_eq = RX_Tx_Payload_2_Frequency_Eq(data_idx); % [1 x N_data]
RX_data_2_demod = pskdemod(RX_data_2_eq, 4, pi/4);
...
RX_Tx_Payload_1 = RX_data_1_eq;
RX_Tx_Payload_2 = RX_data_2_eq;
...
if isfield(Parameters_struct, 'data_Payload_Tx1_1') && isfield(Parameters_struct, 'data_Payload_Tx1_2')
    ...
    len_match = min([length(data_Tx1_1_ref), length(RX_data_1_demod)]);
    errors_Tx1_1 = abs(sign(data_Tx1_1_ref(1:len_match) - RX_data_1_demod(1:len_match)));

    len_match = min([length(data_Tx1_2_ref), length(RX_data_2_demod)]);
    errors_Tx1_2 = abs(sign(data_Tx1_2_ref(1:len_match) - RX_data_2_demod(1:len_match)));

    Error_bits_Tx1 = sum(errors_Tx1_1) + sum(errors_Tx1_2);
    Total_bits_Tx1 = length(errors_Tx1_1) + length(errors_Tx1_2);
    BER_Tx = Error_bits_Tx1 / Total_bits_Tx1;
else
    BER_Tx = NaN; % No reference data available
end
